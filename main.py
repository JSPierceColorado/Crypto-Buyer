...
def append_logs(ws, rows: List[List[str]]):
    ...
    try:
        for i in range(0, len(fixed), 100):
            ws.append_rows(fixed[i:i+100], value_input_option="RAW", table_range=LOG_TABLE_RANGE)
    except TypeError:
...
def append_cost_rows(ws, rows: List[List[str]]):
    ...
    try:
        for i in range(0, len(fixed), 100):
            ws.append_rows(fixed[i:i+100], value_input_option="RAW", table_range=COST_TABLE_RANGE)
    except TypeError:
        start = len(ws.get_all_values()) + 1
        ws.update(f"A{start}:E{start+len(fixed)-1}", fixed, value_input_option="RAW")
...
def list_portfolios() -> List[Dict[str, str]]:
    ...
    for p in items:
        ...
        name = (g(p, "name", "portfolio_name") or "").strip()
        if pid: out.append({"id": pid, "name": name})
    return out

def accounts_for_portfolio(portfolio_uuid: Optional[str] = None):
    return CB.get_accounts(portfolio_uuid=portfolio_uuid) if portfolio_uuid else CB.get_accounts()
...
def resolve_portfolio_uuid() -> Optional[str]:
    if PORTFOLIO_ID: return PORTFOLIO_ID
    if PORTFOLIO_NAME:
...
def pick_portfolio_with_usd_if_needed() -> Optional[str]:
    ...
    if usd_cfg > 0: return configured
    try: ports = list_portfolios()
    except Exception: ports = []
    best_id, best_usd = None, 0.0
    for p in ports:
        pid = p["id"]; usd = usd_available_for_portfolio(pid)
        if usd > best_usd: best_id, best_usd = pid, usd
    return configured
...
def place_buy(product_id: str, usd: float) -> str:
    ...
    o = CB.market_order_buy(client_order_id=client_order_id, product_id=product_id, quote_size=f"{usd:.2f}")
    return g(o, "order_id", "id", default=client_order_id)

def poll_fills_sum(order_id: str) -> Dict[str, float]:
    if DRY_RUN: return {"base_qty": 0.0, "fill_usd": 0.0}
        ...
        try:
            ...
            if fills:
                ...
                for x in fills:
                    ...
                    if qv is not None: fill_usd += float(qv)
                    else:
                        px = float(g(x, "price", default=0) or 0); fill_usd += px * sz
                    base_qty += sz
                fee = _fees_sum(fills)
    ...
    if vals and len(vals) > 1:
        for r in range(1, len(vals)):
            ...
            if (row[0] if row else "").strip().upper() == product:
                ...
                ws.update(range_name=f"A{r+1}:E{r+1}",
...
def main():
    ...
    for pid in products:
        try:
            ...
            logs.append([now_iso(), "CRYPTO-BUY", pid, f"{notional:.2f}",
